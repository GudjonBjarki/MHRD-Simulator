...
class SchematicLibrary:
    ...
    def add_schematic(self, schematic: Schematic) -> None:
        ...
        if self.get_schematic_or_none(schematic.schematic_id) is not None:
            raise Exception(
                f"Cannot add schematic with duplicate id: {schematic.schematic_id}"
            )

        self.schematics.append(schematic)

    def simulate_schematic(
        self, schematic: Schematic, input_signals: dict[OutputPinId, bool]
    ) -> dict[InputPinId, bool]:
        ...
        connection_signal_state: dict[Connection, bool | None] = {
            connection: None for connection in schematic.connections
        }

        # Resolve the input signals for the schematic from the input_signals dict.
        circuit_input_connections = schematic.get_output_connections_for_component(
            SchematicInput()
        )
        ...
        while True:
            ...
            for component_id, component_schematic in components.items():
                ...
                component_output_connections = (
                    schematic.get_output_connections_for_component(component_id)
                )
                if are_connections_resolved(
                    schematic, component_output_connections, connection_signal_state
                ):
                    continue

                # If all the input signals for the component are resolved we're able to simulate it.
                component_input_connections = (
                    schematic.get_input_connections_for_component(component_id)
                )
                if are_connections_resolved(
                    schematic, component_input_connections, connection_signal_state
                ):
                    ...
                    component_simulation_output_signals = self.simulate_schematic(
                        component_schematic, component_simulation_input_signals
                    )

                    # Update the signal state for the component's output signals
                    for connection in component_output_connections:
                        connection_signal_state[
                            connection
                        ] = component_simulation_output_signals[
                            InputPinId(connection.source.pin)
                        ]

            # If all the output signals for the schematic are resolved we're done.
            circuit_output_signals = schematic.get_input_connections_for_component(
                SchematicOutput()
            )
            if are_connections_resolved(
                schematic, circuit_output_signals, connection_signal_state
            ):
                return {
...
def nand_logic(input_signals: dict[OutputPinId, bool]):
    ...
    return {
        InputPinId("out"): (
            input_signals[OutputPinId("in1")] and input_signals[OutputPinId("in2")]
        )
    }


def verify_schematic_signal_pins(
    schematic: Schematic, input_signals: dict[OutputPinId, bool]
) -> None:
    """
    ...
    for pin_id in input_signals:
        if pin_id not in schematic.input_pins:
            raise Exception(
                f"Input signal {pin_id} is not defined on schematic {schematic.schematic_id}"
            )

    for pin_id in schematic.input_pins:
        if pin_id not in input_signals:
            raise Exception(
                f"Input signal {pin_id} is not provided for schematic {schematic.schematic_id}"
            )


def get_schematic_components(
    schematic: Schematic, library: SchematicLibrary
) -> dict[SchematicComponentId, Schematic]:
    """
    ...
    for component_id, component_schematic_id in schematic.components.items():
        ...
        if component_schematic is None:
            raise Exception(
                f"Schematic {component_schematic_id} required for {schematic.schematic_id} was not found in library."
            )
...
def are_connections_resolved(
    schematic: Schematic,
    connections: list[Connection],
    signal_state: dict[Connection, bool | None],
) -> bool:
    return all(signal_state.get(connection) is not None for connection in connections)
        ...
        [
            ...
            Connection(
                source=OutputAttachmentPoint(
                    component=SchematicInput(), pin=OutputPinId("in1")
                ),
                destination=InputAttachmentPoint(
                    component=SchematicComponentId("nand1"), pin=InputPinId("in1")
                ),
            ),
            # input.in2 -> nand2.in1
            Connection(
                source=OutputAttachmentPoint(
                    component=SchematicInput(), pin=OutputPinId("in2")
                ),
                destination=InputAttachmentPoint(
                    component=SchematicComponentId("nand1"), pin=InputPinId("in2")
                ),
            ),
            # nand1.out -> nand2.in1
            Connection(
                source=OutputAttachmentPoint(
                    component=SchematicComponentId("nand1"), pin=OutputPinId("out")
                ),
                destination=InputAttachmentPoint(
                    component=SchematicComponentId("nand2"), pin=InputPinId("in1")
                ),
            ),
            # nand1.out -> nand2.in2
            Connection(
                ...
                destination=InputAttachmentPoint(
                    component=SchematicComponentId("nand2"), pin=InputPinId("in2")
                ),
            ),
            # nand2.out -> output.out
            Connection(
                source=OutputAttachmentPoint(
                    component=SchematicComponentId("nand2"), pin=OutputPinId("out")
                ),
                destination=InputAttachmentPoint(
                    component=SchematicOutput(), pin=InputPinId("out")
                ),
            ),
        ]
    ),
...
if __name__ == "__main__":
    ...
    results = library.simulate_schematic(
        test_schematic, {OutputPinId("in1"): True, OutputPinId("in2"): True}
    )
